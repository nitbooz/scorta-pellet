...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def add_pellets(call):
        ...
        if amount > 0:
            hass.data[DOMAIN][entry.entry_id]["total_pellets"] += amount
            hass.data[DOMAIN][entry.entry_id]["history"].append(("add", amount))
            await hass.data[DOMAIN][entry.entry_id]["sensor"].async_update_ha_state()

    async def remove_pellets(call):
        """Service to remove pellets from the stock."""
        amount = call.data.get("amount", 0)
        if amount > 0 and amount <= hass.data[DOMAIN][entry.entry_id]["total_pellets"]:
            hass.data[DOMAIN][entry.entry_id]["total_pellets"] -= amount
            hass.data[DOMAIN][entry.entry_id]["history"].append(("remove", amount))
            await hass.data[DOMAIN][entry.entry_id]["sensor"].async_update_ha_state()
